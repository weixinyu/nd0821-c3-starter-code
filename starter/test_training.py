...
def test_data_shape(raw_data):
    ...
    assert raw_data.shape == raw_data.dropna().shape, \
        "Dropping null changes shape."


def test_column_names(raw_data):
    """ Test whether the column names of raw data is right. """
    expected_colums = ['age', 'workclass', 'fnlgt', 'education',
...
def test_process_data(train_data):
    """ Test whether the process data function return the right data shape. """
    X_train, y_train = train_data
